...
learning.rate.plot <- function(learning.rate.values, 
  ...
  p = ggplot() +
  geom_point(data=filter(learning.rate.values,Group==group), aes(x=Session, y=Perc.trend), color=color, size=3) +
  geom_line(data=filter(learning.rate.values,Group==group), aes(x=Session, y=Perc.trend, group = 0), color=color, size= 1) +
  geom_errorbar(data=filter(learning.rate.values,Group==group), aes(x=Session, y=Perc.trend, ymin=lower.CL, ymax=upper.CL), width=.2,
