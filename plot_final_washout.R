...
plot.final.washout <- function(data.washout,
  ...
  p = ggplot() +
    geom_point(data=filter(data.washout, Group==group, Num==final.washout.num), 
               aes(x=Session, y=Asym), 
               alpha = 0.5,
               color = color.light) +
    geom_line(data=filter(data.washout, Group==group, Num==final.washout.num), 
              aes(x=Session, y=Asym, group=Animal), 
              alpha = 0.5,
              color = color.light) +
    
    geom_point(data=filter(data.washout.summary, Group==group, Num==final.washout.num), 
               aes(x=Session, y=Fit), 
               size=3, 
               color=color) +
    geom_line(data=filter(data.washout.summary, Group==group, Num==final.washout.num), 
              aes(x=Session, y=Fit, group=0), 
              size= 1, 
              color=color) +
    geom_errorbar(data=filter(data.washout.summary, Group==group, Num==final.washout.num), 
                  aes(x=Session, y=Fit, ymin=Lower, ymax=Upper), 
                  width=.2,
                  position=position_dodge(0.05),
                  color=color) +
...
if (name == 'Exp3') {
  ...
  (washout.values = as.data.frame(summary(emmeans(
    model.washout, ~ Session * Group,
...
(  plot.final.washout.switch = plot.final.washout(data.washout,
                               data.washout.summary,
                               group=group, 
                               color=color, 
                               xlim=xlim))
