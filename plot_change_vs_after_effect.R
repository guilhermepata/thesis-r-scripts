...
data.split.initial.error <-
  rename(data.split.initial.error,
         Session = x,
         Num = group,
  )
data.split.final.error = as.data.frame(ggpredict(model.split, terms = c("Session", "Num [3]")))
data.split.final.error <-
  rename(data.split.final.error,
         Session = x,
         Num = group, )

change.over.split.animals <-
  filter(data.split, Session == 'S5' & Num == 3)$Asym -
  filter(data.split, Session == 'S1' &
           Num == 0)$Asym
change.over.split.estimate <-
  data.split.final.error[data.split.final.error$Session == 'S5',]$predicted -
  data.split.initial.error[data.split.final.error$Session == 'S1',]$predicted
change.over.split.std.error <-
  sqrt(data.split.final.error[data.split.final.error$Session == 'S5',]$std.error ^
         2 +
         data.split.initial.error[data.split.final.error$Session == 'S1',]$std.error ^
         2)

data.washout.after.effect = as.data.frame(ggpredict(model.washout, terms = c("Session", "Num [0]")))
data.washout.after.effect <-
  rename(data.washout.after.effect,
         Session = x,
         Num = group,
  )

after.effect.animals <-
  filter(data.washout, Session == 'S5' & Num == 0)$Asym
after.effect.estimate <-
  data.washout.after.effect[data.washout.after.effect$Session == 'S5',]$predicted
after.effect.std.error <-
  data.washout.after.effect[data.washout.after.effect$Session == 'S5',]$std.error
...
for (animal in unique(data.split$Animal)) {
  df.aux = data.frame(
    ...
    Estimate = c(
      filter(data.split, Session == 'S5' &
               Num == 3 & Animal == animal)$Asym -
        filter(data.split, Session == 'S1' &
                 Num == 0 & Animal == animal)$Asym
    ),
    Animal = c(animal)
  )
...
for (animal in unique(data.washout$Animal)) {
  df.aux = data.frame(
    ...
    Estimate = c(
      filter(data.split, Session == 'S5' & Num == 0 &
               Animal == animal)$Asym
    ),
    Animal = c(animal)
  )
...
(
  p <- ggplot() +
    geom_point(data = df.animals, aes(
      x = Phase,
      y = Estimate,
      group = Animal,
      color = Animal
    )) +
    geom_line(data = df.animals, aes(
      x = Phase,
      y = Estimate,
      group = Animal,
      color = Animal
    ...
    geom_line(
      data = df,
      aes(x = Phase, y = Estimate, group = 0),
      size  = 1
    ) +
