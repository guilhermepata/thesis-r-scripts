...
if (!exists("is.builtLR") || !is.builtLR) {
  ...
  data.first.lr.summary = summarise.predict.old(data.first.lr,
                                                model.first.lr,
                                                Trial,
                                                Perc,
                                                Num,
  ...
  (
    mm_plot <-
      ggplot(data.first.lr, aes(
        x = Perc, y = Asym, color = Group
      )) +
      facet_wrap( ~ Session, nrow = 1) +   # a panel for each session
      geom_point(
        data = data.first.lr,
        aes(x = Perc, y = Asym, group = Animal),
        alpha = 0.5
      ) +
      geom_line(
        data = data.first.lr,
      ...
      theme(panel.spacing = unit(2, "lines"))  # adding space between panels+ggtitle(model.equation.lr)
  )
