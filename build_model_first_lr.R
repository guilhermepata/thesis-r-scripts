...
learning.rate.values[3:7] <- learning.rate.values[3:7] / secs_perc
  ...
  (learning.rate.plot = ggplot() +
    ...
    geom_point(data=filter(learning.rate.values,Group=='NotAtaxic:Switch'), aes(x=Session, y=Perc.trend), size=3) +
    geom_line(data=filter(learning.rate.values,Group=='NotAtaxic:Switch'), aes(x=Session, y=Perc.trend, group = 0), size= 1) +
    geom_errorbar(data=filter(learning.rate.values,Group=='NotAtaxic:Switch'), aes(x=Session, y=Perc.trend, ymin=lower.CL, ymax=upper.CL), width=.2,
...
# for Exp5
{
  ...
  # switch
  (plot.learning.rate.switch = ggplot() +
      # geom_point(data=filter(data.first.lr, Protocol=='Switch', Num==0), aes(x=Session, y=Asym, color=Animal), alpha = 0.2) +
      # geom_line(data=filter(data.first.lr, Protocol=='Switch', Num==0), aes(x=Session, y=Asym, color=Animal, group=Animal), alpha = 0.2) +
      geom_point(data=filter(learning.rate.values,Group=='NotAtaxic:Switch'), aes(x=Session, y=Perc.trend), size=3) +
      geom_line(data=filter(learning.rate.values,Group=='NotAtaxic:Switch'), aes(x=Session, y=Perc.trend, group = 0), size= 1) +
  ...
  # no switch
  (plot.learning.rate.noswitch = ggplot() +
      # geom_point(data=filter(data.first.lr, Protocol=='Switch', Num==0), aes(x=Session, y=Asym, color=Animal), alpha = 0.2) +
      # geom_line(data=filter(data.first.lr, Protocol=='Switch', Num==0), aes(x=Session, y=Asym, color=Animal, group=Animal), alpha = 0.2) +
      geom_point(data=filter(learning.rate.values,Group=='NotAtaxic:NoSwitch'), aes(x=Session, y=Perc.trend), size=3) +
      geom_line(data=filter(learning.rate.values,Group=='NotAtaxic:NoSwitch'), aes(x=Session, y=Perc.trend, group = 0), size= 1) +
      geom_errorbar(data=filter(learning.rate.values,Group=='NotAtaxic:NoSwitch'), aes(x=Session, y=Perc.trend, ymin=lower.CL, ymax=upper.CL), width=.2,
                    position=position_dodge(0.05)) +
      geom_hline(yintercept=c(0), linetype="dotted") +
      xlim('S1', 'S2', 'S3', 'S4', 'S5') +
      theme_classic() + theme(legend.position="none") +
...
# for Exp4
{
  ...
  (plot.learning.rate.ataxic = ggplot() +
      # geom_point(data=filter(data.first.lr, Protocol=='Switch', Num==0), aes(x=Session, y=Asym, color=Animal), alpha = 0.2) +
      # geom_line(data=filter(data.first.lr, Protocol=='Switch', Num==0), aes(x=Session, y=Asym, color=Animal, group=Animal), alpha = 0.2) +
      geom_point(data=filter(learning.rate.values,Group=='Ataxic:Switch'), aes(x=Session, y=Perc.trend), size=3) +
      geom_line(data=filter(learning.rate.values,Group=='Ataxic:Switch'), aes(x=Session, y=Perc.trend, group = 0), size= 1) +
      geom_errorbar(data=filter(learning.rate.values,Group=='Ataxic:Switch'), aes(x=Session, y=Perc.trend, ymin=lower.CL, ymax=upper.CL), width=.2,
                    position=position_dodge(0.05)) +
      geom_hline(yintercept=c(0), linetype="dotted") +
      xlim('S1', 'S2', 'S3', 'S4', 'S5') +
      theme_classic() + theme(legend.position="none") +
